...
def model_wrapper(
    ...
    def get_model_input_time(t_continuous):
        if noise_schedule.schedule == 'discrete':
            # continuous t  ∈ [1/N, 1]  →  discrete step ∈ [0, N-1]
            n = noise_schedule.total_N                        # 1000
            t_input = (t_continuous * n - 1).round()          # 999 … 0
            t_input = t_input.clamp(0, n - 1).to(torch.long)  # int64 にキャスト
            return t_input
        else:
            return t_continuous
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        ...
        # 追加情報付きのタプル / リストを返すモデルに対応
        if isinstance(output, (tuple, list)):
            output = output[0]           # ← ノイズ予測だけ取り出す
        if model_type == "noise":
            return output
...
def dpm_solver_sample(
    model,
    diffusion,                  # guided_diffusion.gaussian_diffusion.GaussianDiffusion
    shape,                      # (B,3,H,W)
    noise=None,
    model_kwargs=None,          # {"physic_cond": ..., "detail_cond": ...}
    steps=30,
    order=2,                    # 1,2,3
    guidance_scale=1.0,
    clip_denoised=True,
    ...
    """
    Wrapper to plug DPM‑Solver++ into scripts/inference.py.

    Returns:
        x_0 in [-1,1] with same shape as `shape`
    """
    device = model.device if hasattr(model, "device") else "cuda"
    model_kwargs = {} if model_kwargs is None else model_kwargs

    # 1) 既学習βスケジュール → NoiseScheduleVP
    # alphas_cumprod = diffusion.alphas_cumprod.to(device)          # shape [T]
    # 1) alphas_cumprod を必ず torch.Tensor にする
    ac = diffusion.alphas_cumprod
    if isinstance(ac, torch.Tensor):
        alphas_cumprod = ac.to(device).float()
    elif isinstance(ac, np.ndarray):
        alphas_cumprod = torch.from_numpy(ac).to(device).float()
    else:
        raise TypeError(f"Unexpected type for alphas_cumprod: {type(ac)}")
    ...
    # 2) classifier‑free guidance ラップ
    #    Diffusion‑Rig の UNet は cond_dict を第3引数で受け取る
    # cond  = model_kwargs
    # “unconditional” 入力はゼロテンソルで十分
    # uncond = {k: torch.zeros_like(v) for k, v in cond.items()}
    """
    cond  = {k: v.clone() for k, v in model_kwargs.items()}   # ← deep copy
    uncond = {k: torch.zeros_like(v) for k, v in cond.items()}
    ...
    model_fn = model_wrapper(
        ...
        ns,
        model_type="noise",
        # model_kwargs={},                   # UNet 側へは cond_dict で渡すので空
        # guidance_type="classifier-free",
        model_kwargs=model_kwargs,      # ← ★ ここを空{} にしない
        guidance_type="uncond",         
        condition=cond,
    ...
    """
    # guidance は使わず、cond は全部 model_kwargs 経由で渡す
    model_fn = model_wrapper(
        model,
        ns,
        model_type="noise",
        model_kwargs=model_kwargs,         # physic_cond と detail_cond をそのまま
        guidance_type="uncond",            # guidance 無し
    )
    ...
    """
    x_T = (
        torch.randn(*shape, device=device, dtype=alphas_cumprod.dtype)
        if noise is None else noise.to(device)
    )
    """
    x_T = (
        torch.randn(*shape, device=device, dtype=torch.float32)
        if noise is None else noise.to(device).float()  # ★ float32 に揃える
    )

    # 離散 DPM なので t_end = 1/num_timesteps が慣例
    x_0 = solver.sample(
        x_T,
        steps=steps,
        t_start=1.0,
        t_end=1.0 / diffusion.num_timesteps,
        order=order,
    ...
    return x_0.clamp(-1, 1) if clip_denoised else x_0