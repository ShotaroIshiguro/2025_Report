...
class Timer:
    ...
    def reset(self):
        self.reset_time = datetime.datetime.now()
        print(f"Timer reset at {self.reset_time}")
    ...
    def run_loop(self):
        ...
        while (
            ...
            if (self.step + self.resume_step) % self.save_interval == 0 and self.step > 0:
                ...
                print("saved")
                keika = timer.elapsed_time()

                message_dict = {
                    "to": LINE_USER_ID,
                    "messages": [
                    {"type": "text", "text": f"model{(self.step+self.resume_step):06d}.pt生成中"},
                    {"type": "text", "text": f"経過時間：{keika}"}
                    ]
                }
                print("message in")
