...
from utils.script_util50_V2 import (
...
def create_inter_data(dataset, modes, meanshape_path="", target_model=None, source_model=None):
    ...
    with th.no_grad():
        if target_model == "EMOCA" or target_model == "emoca" :
            # --- 修正箇所：test関数の戻り値を直接使う ---
            code2, _, _  = test(emoca, img2)
            # --- 修正ここまで ---
        elif target_model == "DECA" or target_model == "deca":
    ...
    for i in range(len(dataset) - 1):
        ...
        with th.no_grad():
            if source_model == "EMOCA" or source_model == "emoca":
                # --- 修正箇所：test関数の戻り値を直接使う ---
                code1, _, _  = test(emoca, img1)
                # --- 修正ここまで ---
            elif source_model == "DECA" or source_model == "deca":
                code1 = deca.encode(img1)
                if 'identity' not in code1:
                    code1['identity'] = th.cat([code1['shape'], code1['tex']], dim=1)

        # --- 変更箇所：顔検出失敗などでidentityが生成されなかった場合、その画像をスキップ ---
        if 'identity' not in code1 or 'identity' not in code2:
            print(f"警告: 画像 {i+1} の処理に失敗しました（顔検出失敗の可能性）。この画像をスキップします。")
            continue
        # --- 変更ここまで ---
